-- Update workflow_step_entity with configuration from planning_steps.json
UPDATE workflow_step_entity wse
SET config = '{
    "title": "Initial Concept",
    "description": "Enter your initial concept for the post here.",
    "inputs": {
        "idea_seed": {
            "type": "textarea",
            "label": "Initial Idea",
...
WHERE wss.name = 'idea'
AND wse.sub_stage_id = wss.id
AND wse.name = 'Main';

-- Add configuration for the title step
INSERT INTO workflow_step_entity (name, sub_stage_id, description, step_order, config)
SELECT 'Title', wss.id, 'Generate a title for your post', 2, '{
    "title": "Title",
    "description": "Generate a title for your post based on the expanded idea.",
    "inputs": {
        "expanded_idea": {
            "type": "textarea",
            "label": "Expanded Idea",
...
FROM workflow_sub_stage_entity wss
WHERE wss.name = 'idea'
ON CONFLICT (sub_stage_id, name) DO UPDATE
