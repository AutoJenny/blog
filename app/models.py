...
class Post(db.Model):
    ...
    sections = relationship('PostSection', backref='post', order_by='PostSection.position')
    ...
    workflow_status = relationship('WorkflowStatus', backref='post', uselist=False)
...
class WorkflowStatus(db.Model):
    ...
    current_stage = db.Column(db.Enum(WorkflowStage), nullable=False, default=WorkflowStage.CONCEPTUALIZATION)
    stage_data = db.Column(db.JSON)  # Stores stage-specific data
    last_updated = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    history = relationship('WorkflowStatusHistory', backref='workflow_status', order_by='WorkflowStatusHistory.created_at.desc()')
...
class WorkflowStatusHistory(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    workflow_status_id = db.Column(db.Integer, db.ForeignKey('workflow_status.id'), nullable=False)
