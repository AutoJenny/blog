...
import { API_CONFIG, buildApiUrl, buildLLMApiUrl, handleApiResponse } from './config/api.js';
...
document.getElementById('config-form')?.addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch(buildLLMApiUrl('/config'), {
            method: 'POST',
            headers: {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadPrompts() {
        fetch(buildLLMApiUrl(API_CONFIG.PATHS.PROMPTS))
            .then(r => handleApiResponse(r))
            .then(templates => {
                ...
                templates.forEach(t => {
                    ...
                    deleteBtn.addEventListener('click', () => {
                        if (confirm('Are you sure you want to delete this prompt template?')) {
                            fetch(buildLLMApiUrl(`${API_CONFIG.PATHS.PROMPTS}/${t.id}`), {
                                method: 'DELETE'
                            })
                                .then(r => r.json())
                                .then(data => {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadPromptParts() {
        const resp = await fetch(buildLLMApiUrl(`${API_CONFIG.PATHS.ACTIONS}/${actionId}`));
    ...
    document.getElementById('add-prompt-part-btn').addEventListener('click', function() {
        ...
        if (!content) return;
        fetch(buildLLMApiUrl(API_CONFIG.PATHS.PROMPT_PARTS), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        .then(data => {
            if (data.id) {
                // Link to action
                fetch(buildLLMApiUrl(`${API_CONFIG.PATHS.ACTIONS}/${actionId}/prompt_parts`), {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    promptPartsList.addEventListener('click', function(e) {
        ...
        if (removeBtn) {
            ...
            fetch(buildLLMApiUrl(`${API_CONFIG.PATHS.ACTIONS}/${actionId}/prompt_parts/${partId}`), { method: 'DELETE' })
                .then(() => loadPromptParts());
        } else if (moveUpBtn) {
            const partId = moveUpBtn.dataset.moveUp;
            fetch(buildLLMApiUrl(`${API_CONFIG.PATHS.ACTIONS}/${actionId}/prompt_parts/${partId}/move`), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (newContent) {
                fetch(buildLLMApiUrl(`${API_CONFIG.PATHS.PROMPT_PARTS}/${partId}`), {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    if (runBtn) {
        runBtn.addEventListener('click', async function() {
            ...
            try {
                const resp = await fetch(buildLLMApiUrl(`${API_CONFIG.PATHS.ACTIONS}/${actionId}/test`), {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
...
(async function() {
  // Utility: fetch field mappings from /api/workflow/fields/mappings
  async function fetchFieldMappings() {
    const resp = await fetch('/api/workflow/fields/mappings');
  ...
  async function fetchPostDevelopment(postId) {
    const resp = await fetch(buildPostApiUrl(postId, API_CONFIG.PATHS.DEVELOPMENT));
    return resp.ok ? await resp.json() : {};
  }
  ...
  async function fetchLLMActions() {
    const resp = await fetch(buildLLMApiUrl(API_CONFIG.PATHS.ACTIONS));
    return resp.ok ? await resp.json() : [];
  }
...
// Update all API endpoints to use the new standardized paths
const API_ENDPOINTS = {
    PROMPTS: '/api/llm/prompts',
    ACTIONS: '/api/llm/actions',
    PROMPT_PARTS: '/api/llm/prompt_parts',
    POST_DEVELOPMENT: '/api/blog/posts',
};

// Replace all fetch calls with the new endpoints
fetch(API_ENDPOINTS.PROMPTS)
// ... existing code ...
fetch(`${API_ENDPOINTS.PROMPTS}/${t.id}`, {
// ... existing code ...
fetch(`${API_ENDPOINTS.PROMPTS}/${t.id}`, {
// ... existing code ...
fetch(API_ENDPOINTS.PROMPTS, {
// ... existing code ...
const resp = await fetch(`${API_ENDPOINTS.ACTIONS}/${actionId}`);
// ... existing code ...
fetch(API_ENDPOINTS.PROMPTS, {
// ... existing code ...
const resp = await fetch(`${API_ENDPOINTS.ACTIONS}/${actionId}`);
// ... existing code ...
const resp = await fetch(`${API_ENDPOINTS.ACTIONS}/${actionId}/test`, {
// ... existing code ...
const resp = await fetch(`${API_ENDPOINTS.POST_DEVELOPMENT}/${postId}/development`);
// ... existing code ...
const resp = await fetch(`${API_ENDPOINTS.POST_DEVELOPMENT}/${postId}/development`, {
// ... existing code ...
const resp = await fetch(API_ENDPOINTS.ACTIONS);
// ... existing code ...
const resp = await fetch(`${API_ENDPOINTS.ACTIONS}/${actionId}`);
// ... existing code ...
const resp = await fetch(`${API_ENDPOINTS.ACTIONS}/${actionId}/test`, {
...
fetch(API_ENDPOINTS.PROMPTS, {
// ... existing code ...
fetch(`${API_ENDPOINTS.ACTIONS}/${actionId}/prompt_parts`, {
// ... existing code ...
fetch(`${API_ENDPOINTS.ACTIONS}/${actionId}/prompt_parts/${partId}`, { method: 'DELETE' })
// ... existing code ...
fetch(`${API_ENDPOINTS.ACTIONS}/${actionId}/prompt_parts/${partId}/move`, {
// ... existing code ...
fetch(`${API_ENDPOINTS.ACTIONS}/${actionId}/prompt_parts/${partId}/move`, {
// ... existing code ...
fetch(`${API_ENDPOINTS.PROMPTS}/${partId}`, {
// ... existing code ...
const resp = await fetch(`${API_ENDPOINTS.ACTIONS}/${actionId}`);
// ... existing code ...
const resp = await fetch(`${API_ENDPOINTS.ACTIONS}/${actionId}/test`, {
