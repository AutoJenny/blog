...
def resolve_target_post_id(post_id, year=None, week=None, require_week=False):
    """
    Validate week context if required, but ALWAYS return post_id from URL.
    
    CRITICAL: post_id in URL is the definitive identifier.
    Week parameters are context only, not for changing post_id.
    
    Args:
        post_id: Original post_id from URL (ALWAYS used as target_post_id)
        year: Year from query params (optional, for validation/context only)
        week: Week from query params (optional, for validation/context only)
        require_week: If True, return error if year/week not provided (only for themed posts)
    
    Returns:
        tuple: (target_post_id, error_message)
        - target_post_id is ALWAYS the post_id from URL (never changed)
        - error_message is set if week context is required but missing
    """
    from utils.taxonomy_helpers import get_post_type
    ...
    # For themed posts, validate week context if required, but NEVER change post_id
    if require_week and (not year or not week):
        return (post_id, 'Week context (year and week) is required for themed posts')
    
    # ALWAYS return post_id from URL - never change it based on week
    return (post_id, None)
