        ...
        function fetchLLMProviders() {
            ...
            fetch('/api/syndication/llm/providers')
                .then(response => response.json())
                .then(data => {
                    console.log('LLM providers data:', data);
                    const providerSelect = document.getElementById('llmProviderSelect');
                    console.log('Provider select element after fetch:', providerSelect);
                    if (providerSelect && data.providers) {
                        ...
                        providerSelect.addEventListener('change', function() {
                            ...
                            if (selectedProviderId) {
                                // Enable model select and fetch models
                                modelSelect.disabled = false;
                                modelSelect.innerHTML = '<option value="">Loading models...</option>';
                                fetchLLMModels(selectedProviderId);
                            } else {
        ...
        // Fetch LLM Models for a specific provider
        function fetchLLMModels(providerId) {
            console.log('Fetching LLM models for provider:', providerId);
            fetch(`/api/syndication/llm/models/${providerId}`)
                .then(response => response.json())
                .then(data => {
                    console.log('LLM models data:', data);
                    const modelSelect = document.getElementById('llmModelSelect');
        ...
        // Manual function for debugging - can be called from browser console
        window.debugLLMProviders = function() {
            console.log('Manual LLM provider debug...');
            fetchLLMProviders();
        };
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Also try to initialize LLM settings after a longer delay
            setTimeout(() => {
                console.log('Delayed LLM initialization...');
                const providerSelect = document.getElementById('llmProviderSelect');
                if (providerSelect && providerSelect.innerHTML.includes('Loading providers')) {
                    console.log('Provider select still loading, retrying...');
                    fetchLLMProviders();
                }
            }, 1000);
