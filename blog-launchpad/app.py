...
@app.route('/api/syndication/facebook/test-post', methods=['POST'])
def facebook_test_post():
    """Test post to Facebook (as draft or to test page)."""
    try:
        data = request.get_json()
        content = data.get('content', '')
        image_url = data.get('image_url', '')
        test_mode = data.get('test_mode', 'draft')  # 'draft' or 'test_page'
        ...
        with get_db_connection() as conn:
            ...
            if not creds.get('page_access_token'):
                return jsonify({'success': False, 'error': 'Page Access Token not configured'})
            
            if not creds.get('page_id'):
                return jsonify({'success': False, 'error': 'Page ID not configured'})
        
        # Prepare Facebook API call
        page_id = creds['page_id']
        access_token = creds['page_access_token']
        
        # Facebook Graph API endpoint - use feed posts
        # Note: Facebook doesn't support draft posts via API, so we'll post live but mark as test
        url = f"https://graph.facebook.com/v18.0/{page_id}/feed"
        
        # Add test indicator to content
        test_content = content
        if test_mode == 'draft':
            test_content = f"🧪 TEST POST - {content}\n\n(This is a test post from Blog Launchpad)"
        
        payload = {
            'message': test_content,
            'published': True,  # Facebook requires published posts via API
        ...
        if response.status_code == 200:
            ...
            return jsonify({
                'success': True,
                'message': f'Test post created successfully! Mode: {test_mode}',
        ...
        logger.error(f"Error in facebook_test_post: {e}")
        return jsonify({'success': False, 'error': str(e)})
