...
class PathResolver:
    ...
    def convert_web_path_to_filesystem(self, web_path):
        """Convert a web path to a file system path. ALL images are in project_root/static/"""
        if web_path.startswith('/static/'):
            # ALL images are in project_root/static/ - no fallbacks
            fs_path = os.path.join(self.project_root, web_path[1:])  # Remove leading '/'
            return fs_path
            ...
            fs_path = os.path.join(self.project_root, 'static', relative_path)
            return fs_path
        else:
