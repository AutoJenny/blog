...
def plan_structure_llm():
    ...
    try:
        ...
        # Plan the structure
        sections = llm_service.plan_structure(
...
@bp.route('/api/v1/structure/save/<int:post_id>', methods=['POST'])
def save_structure(post_id):
    ...
    try:
        ...
        cur.execute('DELETE FROM post_sections WHERE post_id = %s', (post_id,))

        # Insert new sections
        for section in data['sections']:
            cur.execute('''
                INSERT INTO post_sections (post_id, title, description, ideas_to_include, facts_to_include)
                VALUES (%s, %s, %s, %s, %s)
            ''', (
                post_id,
                section['title'],
        ...
        if conn:
            conn.rollback()
        current_app.logger.error(f'Error saving structure: {str(e)}')
        return jsonify({'error': str(e)}), 500
    finally:
        if cur:
            cur.close()
