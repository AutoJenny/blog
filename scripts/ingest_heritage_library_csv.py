#!/usr/bin/env python3
"""
Ingest Heritage/Lineage CSV into heritage_library.

Reads a CSV with columns: category, title, body_text, tone, source_note, image_idea, confidence_flag, image_style.
Inserts into heritage_library (category, title, body_text, source_note, active=TRUE).
Extra columns are not stored; schema matches culture_library.
"""
...
def ensure_heritage_library_exists() -> bool:
    """Create heritage_library table if it does not exist. Returns True if table exists (or was created)."""
    with db_manager.get_cursor() as cursor:
        cursor.execute(
            """
            SELECT 1 FROM information_schema.tables
            WHERE table_schema = 'public' AND table_name = 'heritage_library'
            """
        )
        if cursor.fetchone():
            return True
...
def ingest_csv(csv_path: str, dry_run: bool = False, run_migration: bool = False) -> dict:
    """
    Read CSV and insert rows into heritage_library.
    ...
    if run_migration and not dry_run:
        ensure_heritage_library_exists()
        ...
        for row in reader:
            ...
            try:
                with db_manager.get_cursor() as cursor:
                    cursor.execute(
                        """
                        INSERT INTO heritage_library (category, title, body_text, source_note, active)
                        VALUES (%s, %s, %s, %s, TRUE)
                        """,
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Ingest heritage_library from CSV.")
    parser.add_argument(
        "csv_path",
        nargs="?",
        default="docs/HERITAGE_LINEAGE_v1_0_FINAL_INGESTION_PATCHED.csv",
        help="Path to CSV (default: docs/HERITAGE_LINEAGE_v1_0_FINAL_INGESTION_PATCHED.csv)",
    )
    parser.add_argument("--dry-run", action="store_true", help="Count rows only, do not insert")
    parser.add_argument(
        "--run-migration",
        action="store_true",
        help="Create heritage_library table if it does not exist (then ingest)",
    )
