...
def workflow_prompts():
    ...
    # Get database configuration for the template
    from dotenv import dotenv_values
    import os
    # Get the path to the assistant_config.env file relative to the project root
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
    config_path = os.path.join(project_root, 'assistant_config.env')
    ...
    try:
        ...
        with get_db_conn() as conn:
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                ...
                # Create system prompt for the new step
                system_prompt_name = f"{data['name'].upper()} system"
                system_prompt_text = "You are an expert in Scottish history and culture, dedicated to accuracy and authenticity in everything you do. You adhere to academic values, but love to popularise ideas to make them easily understandable to those with no knowledge of your specialism."
                
                cur.execute("""
                    INSERT INTO llm_prompt 
                    (name, description, prompt_text, system_prompt, "order", step_id)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    RETURNING id
                """, (system_prompt_name, f"System prompt for {data['name']}", system_prompt_text, system_prompt_text, 1, new_step_id))
                
                system_prompt_id = cur.fetchone()['id']
                
                # Create task prompt for the new step
                task_prompt_name = f"{data['name'].upper()} task"
                task_prompt_text = f"Process the input content below according to the requirements for the {data['name']} step. Return only the processed content, with no additional commentary or formatting."
                ...
                cur.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s)
                    RETURNING id
                """, (task_prompt_name, f"Task prompt for {data['name']}", task_prompt_text, system_prompt_text, 2, new_step_id))
                
                task_prompt_id = cur.fetchone()['id']
                
                # Link the prompts to the step
                cur.execute("""
                    INSERT INTO workflow_step_prompt 
                    (step_id, system_prompt_id, task_prompt_id)
                    VALUES (%s, %s, %s)
                """, (new_step_id, system_prompt_id, task_prompt_id))
                
                conn.commit()
                
                return jsonify({'success': True, 'step_id': new_step_id, 'message': f'Step "{data["name"]}" created successfully with prompts'})
